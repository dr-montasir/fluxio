...
use fluxio::client::HttpConnector;
use fluxio::{body::HttpBody as _, Body, Method, Request, Response, Server};
...
impl Opts {
    ...
    fn bench(self, b: &mut test::Bencher) {
        ...
        let client = fluxio::Client::builder()
        ...
        let url: fluxio::Uri = format!("http://{}/hello", addr).parse().unwrap();
    ...
    let srv = rt.block_on(async move {
        Server::bind(&addr)
            ...
            .serve(make_service_fn(move |_| async move {
                Ok::<_, fluxio::Error>(service_fn(move |req: Request<Body>| async move {
                    let mut req_body = req.into_body();
                    while let Some(_chunk) = req_body.data().await {}
                    Ok::<_, fluxio::Error>(Response::new(Body::from(body)))
                }))
            }))
    });
