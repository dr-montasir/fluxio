...
# This script regenerates fluxio.h. As of April 2021, it only works with the
...
header_file_backup="$CAPI_DIR/include/fluxio.h.backup"
...
cp "$CAPI_DIR/include/fluxio.h" "$header_file_backup"
...
if ! output=$(RUSTFLAGS='--cfg fluxio_unstable_ffi' cargo rustc -- -Z unpretty=expanded 2>&1 > expanded.rs); then
    # As of April 2021 the script above prints a lot of warnings/errors, and
    # exits with a nonzero return code, but fluxio.h still gets generated.
    #
...
if ! cbindgen \
    ...
    --output "$CAPI_DIR/include/fluxio.h" \
    "${@}"; then
    bindgen_exit_code=$?
    if [[ "--verify" == "$1" ]]; then
        echo "diff generated (<) vs backup (>)"
        diff "$CAPI_DIR/include/fluxio.h" "$header_file_backup"
    fi
