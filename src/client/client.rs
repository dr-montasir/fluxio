...
use crate::common::{
    exec::BoxSendFuture, lazy as fluxio_lazy, sync_wrapper::SyncWrapper, task, Future, Lazy, Pin,
    Poll,
};
...
impl Client<(), Body> {
    ...
    ///     .http2_only(true)
    ///     .build_http();
    /// # let infer: Client<_, fluxio::Body> = client;
    /// # drop(infer);
    /// # }
    /// # fn main() {}
    /// ```
    ...
    B::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    /// # #[cfg(feature  = "runtime")]
    /// # fn run () {
    /// use fluxio::{Body, Method, Client, Request};
    ///
    /// let client = Client::new();
    ///
...
impl Builder {
    ...
    ///
    /// Setting this to true will force fluxio to use queued strategy
    /// which may eliminate unnecessary cloning on some TLS backends
    ///
    /// Default is `auto`. In this mode fluxio will try to guess which
    /// mode to use
    ...
    ///
    /// If not set, fluxio will use a default.
    ///
