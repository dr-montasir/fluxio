...
//! # fluxio C API
//!
//! This part of the documentation describes the C API for fluxio. That is, how
//! to *use* the fluxio library in C code. This is **not** a regular Rust
//! module, and thus it is not accessible in Rust.
//!
//! ## Unstable
//!
//! The C API of fluxio is currently **unstable**, which means it's not part of
//! the semver contract as the rest of the Rust API is. Because of that, it's
//! only accessible if `--cfg fluxio_unstable_ffi` is passed to `rustc` when
...
//!
//! ```notrust
//! RUSTFLAGS="--cfg fluxio_unstable_ffi" cargo build --features client,http1,http2,ffi
//! ```
...
#[cfg(not(fluxio_unstable_ffi))]
compile_error!(
    "\
    The `ffi` feature is unstable, and requires the \
    `RUSTFLAGS='--cfg fluxio_unstable_ffi'` environment variable to be set.\
"
);
...
#[allow(unused)]
pub const FLUXIO_ITER_BREAK: libc::c_int = 1;

/// An HTTP Version that is unspecified.
pub const FLUXIO_HTTP_VERSION_NONE: libc::c_int = 0;
/// The HTTP/1.0 version.
pub const FLUXIO_HTTP_VERSION_1_0: libc::c_int = 10;
/// The HTTP/1.1 version.
pub const FLUXIO_HTTP_VERSION_1_1: libc::c_int = 11;
/// The HTTP/2 version.
pub const FLUXIO_HTTP_VERSION_2: libc::c_int = 20;
...
ffi_fn! {
    ...
    fn fluxio_version() -> *const libc::c_char {
