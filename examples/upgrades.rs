...
// Note: `fluxio::upgrade` docs link to this upgrade.
use std::str;

use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::sync::oneshot;

use fluxio::header::{HeaderValue, UPGRADE};
use fluxio::service::{make_service_fn, service_fn};
use fluxio::upgrade::Upgraded;
use fluxio::{Body, Client, Request, Response, Server, StatusCode};
...
async fn server_upgrade(mut req: Request<Body>) -> Result<Response<Body>> {
    ...
    tokio::task::spawn(async move {
        match fluxio::upgrade::on(&mut req).await {
            Ok(upgraded) => {
...
async fn client_upgrade_request(addr: SocketAddr) -> Result<()> {
    ...
    match fluxio::upgrade::on(res).await {
        Ok(upgraded) => {
...
async fn main() {
    ...
    let make_service =
        make_service_fn(|_| async { Ok::<_, fluxio::Error>(service_fn(server_upgrade)) });
