...
async fn index1(_: Request<Body>) -> Result<Response<Body>, fluxio::Error> {
    Ok(Response::new(Body::from(INDEX1)))
}

async fn index2(_: Request<Body>) -> Result<Response<Body>, fluxio::Error> {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let srv1 = Server::bind(&addr1).serve(make_service_fn(|_| async {
        Ok::<_, fluxio::Error>(service_fn(index1))
    }));

    let srv2 = Server::bind(&addr2).serve(make_service_fn(|_| async {
        Ok::<_, fluxio::Error>(service_fn(index2))
    }));
