...
async fn param_example(req: Request<Body>) -> Result<Response<Body>, fluxio::Error> {
    match (req.method(), req.uri().path()) {
        (&Method::GET, "/") | (&Method::GET, "/post") => Ok(Response::new(INDEX.into())),
        (&Method::POST, "/post") => {
            // Concatenate the body...
            let b = fluxio::body::to_bytes(req).await?;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let server = Server::bind(&addr).serve(make_service_fn(|_| async {
        Ok::<_, fluxio::Error>(service_fn(param_example))
    }));
